...
class ProductListLayout extends Component {
  ...
  onContextMenuClick = (e, data, target) => {
    if (data.action === 'delete'){
      let comp_this = this;
      axios.defaults.headers.delete["Authorization"] = localStorage.getItem('session_key');
      axios.delete(piranhaConhandleCheckChangefig.apiBaseUrl + piranhaConfig.route.products + '/' + this.state.selectedItems)
    ...
    if (data.action === 'view') {
      this.props.history.push('product-details/' + data.data)
    }
  };
  ...
  dataListRender() {
    ...
    axios.get(piranhaConfig.apiBaseUrl + piranhaConfig.route.products)
      .then(function (response) {
  ...
  addProduct = () => {
    ...
    if (product.name === "") {
      this.setState({ addProductError: "***Product Name Required***" });
      return;
    }
    let comp_this = this;

    axios.defaults.headers.post["Authorization"] = localStorage.getItem('session_key');
    axios.post(piranhaConfig.apiBaseUrl + piranhaConfig.route.products, product)
      .then(function (response) {
        comp_this.dataListRender();
                  ...
                  <div className="float-sm-right">
                    ...
                    >
                      ...
                      <ModalBody>
                        ...
                        <Input defaultValue={this.state.newProductName} onChange={this.onProductAddChange} />
                          ...
                          <div className="position-relative">
                            ...
                            >
                              <CardImg
                                top
                                alt={product.name}
                                src={'https://support.apple.com/library/content/dam/edam/applecare/images/en_US/il/ios12-product-lockup-callout.png'}
                              />
                            </NavLink>
                            <Badge
                            ...
                            >
                              {product.created_by.forename + ' ' + product.created_by.surname}
                            </Badge>
                          </div>
                          <CardBody>
                            <Row>
...
export default injectIntl(mouseTrap(ProductListLayout))
